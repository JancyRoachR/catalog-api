...
from .marcfieldset import filters as filt
...
# MARC FIELD --> UNTBIB PARSERS
# Below are the primary parsers you'll use in conversion functions.
# They each take a `marcfieldset.Field` object and return some data.
...
def person_name(field):
    ...
    sub_a = field.subfields_where(filt.tag_equals, 'a').get_subfields_as_string()
    if has_comma_in_middle(sub_a):
        (surname, forename) = re.split(r',\s*', sub_a, 1)
    else:
        is_forename = True if field['indicators'][0] == '0' else False
        is_surname = True if field['indicators'][0] == '1' else False
        is_family_name = True if field['indicators'][0] == '3' else False
        if is_forename:
            forename = field.subfields_where(filt.tag_in, 'ab').get_subfields_as_string()
        else:
            surname = sub_a
...
def person_titles(field):
    """
    Parse personal name titles (one or more, separated by commas).
    """
    sub_c = field.subfields_where(filt.tag_equals, 'c').get_subfields_as_string()
    return [strip_ends(title) for title in re.split(r',\s*', sub_c) if title]


def _get_person_date_qualifier(obj):
    """
    Helper function for describing a ParsedDate object.
    """
    if obj.qualifier.lower() in ['ca', 'circa']:
        return 'circa'
    if obj.date2 or obj.unsure:
        return 'unsure'
    return ''


def person_dates(field):
    """
    Get and parse dates associated with personal names.
    """
    cent_re = r'\b(\d{1,2})(?=.*cent(ury)?\b)'
    bce_re = r'b\.?c\.?e?\.?'
    date_re = r'\b(([a-zA-Z]+)\W*)?(\d+)(\W*?(\/|or)\W*(\d+)\W*)?(\W*({}))?(\?)?'.format(bce_re)
    date_parts = ['ig1', 'qualifier', 'date1', 'ig2', 'ig3', 'date2', 'ig4', 'era', 'unsure']
    months = ['jan', 'feb', 'mar', 'apr', 'may','jun', 'jul', 'aug', 'sep', 'oct', 'nov', 'dec']
    ParsedCentury = collections.namedtuple('ParsedCentury', ['century', 'ig1'])
    ParsedDate = collections.namedtuple('ParsedDate', date_parts)

    start, end,  = ('', '') 
    start_qualifier, end_qualifier, dtype = ('', '', 'unknown date types')
    sub_d = strip_ends(field.subfields_where(filt.tag_equals, 'd').get_subfields_as_string())
    centuries = [ParsedCentury(*t) for t in re.findall(cent_re, sub_d, re.I)]
    if centuries:
        era_mult, era_sub = (-1, 0) if re.search(bce_re, sub_d, re.I) else (1, 1)
        centnums = [int(c.century) * era_mult for c in centuries]
        start = (centnums[0] - era_sub) * 100
        end = (centnums[-1] * 100) - 1
        dtype = 'approximate centuries'
    else:
        pdates = [ParsedDate(*t) for t in re.findall(date_re, sub_d, re.I)]
        dates = [d for d in pdates if d.qualifier.lower() not in months]
        if dates:
            era_dates = [int(d.date1) for d in dates]
            era_mults = [-1 if d.era else 1 for d in dates]
            if dates[0].qualifier.lower() in ['d', 'died', 'death']:
                end = era_dates[0] * era_mults[0]
                end_qualifier = _get_person_date_qualifier(dates[0])
            else:
                start = era_dates[0] * era_mults[0]
                start_qualifier = _get_person_date_qualifier(dates[0])
                if len(dates) > 1:
                    end = era_dates[1] * era_mults[1]
                    end_qualifier = _get_person_date_qualifier(dates[1])
            if dates[0].qualifier.lower() in ['fl', 'flourished']:
                dtype = 'dates flourished'
            else:
                dtype = 'dates lived'

    return {'start_date': start,
            'start_date_qualifier': start_qualifier,
            'end_date': end,
            'end_date_qualifier': end_qualifier,
            'date_type': dtype,
            'full_dates': sub_d}
...
def untbib_person_get_name_straight(person):
    """
    Return a formatted string based on the untbib `person` data passed.

    Formats name in straight (First Last) order.
    """
    return '{} {}'.format(person['forename'], person['surname']).strip()


def untbib_person_get_name_inverted(person):
    """
    Return a formatted string based on the untbib `person` data passed.

    Formats name in inverted (Last, First) order.
    """
    return '{}, {}'.format(person['surname'], person['forename']).strip(', ')


def untbib_person_get_fullname(person):
    """
    Return a formatted string based on the untbib `person` data passed.

    Formats name as 'First Last, Titles (dates)'
    """
    straightname = untbib_person_get_name_straight(person)
    name_with_titles = '{}, {}'.format(straightname, ', '.join(person['titles'])).strip(', ')
    name_with_dates = '{} ({})'.format(name_with_titles, person['full_dates']) if person['full_dates'] else name_with_titles
